...
const LoginSystemUser = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if ([validOtp[0]["email"], validOtp[0]["otp"], validOtp[0]["time"]].some(el => !el || el === "")) {
            return res.status(404).json({ status: false, msg: "Can't find Otp" });  // 404 for not found
        }
        ...
        if (diff > 15 || validOtp[0]["otp"] != otp) {
            return res.status(401).json({ status: false, msg: "Invalid Otp" });  // 401 for unauthorized access
        }
