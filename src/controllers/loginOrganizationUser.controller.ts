...
const LoginOrganizationUser = async (req: Request, res: Response) => {
  ...
  if (![email, otp].every((el) => typeof el === 'string' && el.trim() !== "")) {
    return res.status(400).json({ success: false, msg: "Fill the details" });
  }

  try {
    const user = await OrganizationUser.find({ email });

    if (user.length !== 1) {
      return res.status(401).json({ success: false, msg: "Invalid Login Credentials" });
    }
    ...
    if (
      ...
      return res.status(404).json({ success: false, msg: "Can't find Otp" });
    }
